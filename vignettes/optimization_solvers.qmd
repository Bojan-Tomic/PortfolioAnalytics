...
solvers** that exploit problem structure for exact solutions, **metaheuristic
...
|--------|---------|--------|----------------|
| DEoptim | `DEoptim` | Differential Evolution | `itermax`, `NP`, `strategy` (default: JADE) |
...
back to feasible space after each mutation step, while GenSA and pso rely on
internal normalization via `fn_map()` inside `constrained_objective()`.
...
and evaluates them all, selecting the best.
...
| Constraint | CVXR | ROI | osqp | Rglpk | Metaheuristic |
|-----------|:----:|:---:|:----:|:-----:|:------------:|
| Box (min/max per asset) | Yes | Yes | Yes | Yes | Yes (penalty) |
...
Metaheuristic solvers require slightly relaxed leverage constraints because
...
VaR and the second moment of losses beyond VaR.  This is an SOCP problem
